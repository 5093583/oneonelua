...
print("fffff" .. tostring(self._unZip))
...
function WelcomeScene:updateApp()
    self._txtTips:setString("更新客户端ing...")
    QueryDialog:create("有新的版本，是否现在下载升级？",function(bConfirm)
	        ...
	        if bConfirm == true then       
                ...
                if device.platform == "ios" then
                    ...
                    self._txtTips:setString("开始下载客户端ing...")
                    cc.Application:getInstance():openURL(self._androidUpdateUrl)
                     self._txtTips:setString("下载客户端end...")
                end
		    else
			    os.exit(0)
	        end					
...
function WelcomeScene:updateRes()
    ...
    if #self._updateQueue == 0 then
        ...
        self._txtTips:setString("更新资源ing...")
        --下载当前项
...
function WelcomeScene:requestServerConfig()
    ...
    print(888888888)
    dump(datatable,10,10)
	 	local succeed = false
	 	local msg = "网络获取失败！"
        self._txtTips:setString("检查更新...")
        ...
        if succeed then --成功
            ...
            --判断是否需要重新下载完整App
 	        if self._newVersion and self._newVersion > appdf.BASE_C_VERSION then
                --更新客户端
                self._txtTips:setString("更新客户端...")
                self:updateApp()
            else
                --更新资源
                self._txtTips:setString("更新资源...")
                self:updateRes()
