...
function QueryDialog:ctor(msg, callback, txtsize, queryType)
    ...
    ccui.Text:create()
             :setString("提  示")
             :setFontSize(36)
             :setColor(cc.c3b(255,249,178))
		     :setFontName("fonts/round_body.ttf")
		     :move(contentSize.width/2 , 455)
		     :addTo(self._content)
	...
	if QueryDialog.QUERY_SURE == queryType then
		ccui.Button:create("bt_query_confirm_0.png","bt_query_confirm_1.png")
		    :setTitleText("确定")
		    :setTitleFontSize(32)
		    :setTitleColor(cc.c3b(255,255,255))
...
--		:addTo(self)
	...
	cc.Label:createWithTTF(msg, "fonts/round_body.ttf", not txtsize and QueryDialog.DEF_TEXT_SIZE or txtsize)
		:setTextColor(cc.c3b(222,197,232))
		:setAnchorPoint(cc.p(0.5,0.5))
		:setDimensions(640, 180)
		:setHorizontalAlignment(cc.TEXT_ALIGNMENT_CENTER)
		:setVerticalAlignment(cc.VERTICAL_TEXT_ALIGNMENT_CENTER)
		:move(contentSize.width/2 ,270 )
		:addTo(self._content)
	self._dismiss  = false
...
--	self:dismiss()
	...
	self:dismiss()
	if callback then
		callback(tag == QueryDialog.BT_CONFIRM)
	end
