...
function ExternalFun.loadRootCSB( csbFile, parent )
	local rootlayer = ccui.Layout:create()
		...
		:setScale(yl.WIDTH / 1334);
	if nil ~= parent then
		parent:addChild(rootlayer);
	end
    print("加截csb33",csbFile)
...
--播放大厅音效
function ExternalFun.playPlazaEffect(file)
	if GlobalUserItem.bSoundAble then
		AudioEngine.playEffect(cc.FileUtils:getInstance():fullPathForFilename("sound/"..file),false)
	end	
end
...
--播放菜单音效
function ExternalFun.playMenuEffect( )
	if GlobalUserItem.bSoundAble then
		AudioEngine.playEffect(cc.FileUtils:getInstance():fullPathForFilename("sound/Menu.mp3"),false)
	end
end

--播放返回音效
function ExternalFun.playReturnEffect( )
	if GlobalUserItem.bSoundAble then
		AudioEngine.playEffect(cc.FileUtils:getInstance():fullPathForFilename("sound/Return.mp3"),false)
	end
end
...
function ExternalFun.playPlazzBackgroudAudio( )
	if GlobalUserItem.bVoiceAble then
		AudioEngine.playMusic(cc.FileUtils:getInstance():fullPathForFilename("sound/background01.mp3"),true)
	end
end
...
print("原始分",score)
	--开关，1返回换算过的比特币，1分等于0.000001比特币，0.001mBTC,1uBTC,2原样返回
	local isbtc = 1
	if isbtc == 2 then
		return score
	end

    --显示值缩小1000倍，以mBTC为单位显示 1000m以上实现btc
    local scorestr = score/1000.0
    print("转mBTC以后",scorestr)
	if scorestr < 1000 then
		scorestr = string.format("%.3fm", scorestr)
		print("小于1000",scorestr)
		return scorestr
	else

		scorestr = string.format("%.3f", scorestr / 1000)
		print("大于1个比特币",scorestr)
		return scorestr
	end
end
...
--检查手机号
function ExternalFun.isPhoneNumber(var)
...
--base64编码
function ExternalFun.base64FromString(source_str)  
...
--base64编码
function ExternalFun.base64FromBytes(source_bytes)  
    local b64chars = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/'  
    local s64 = ''  
    local index = 0
...
--base64解码
function ExternalFun.stringFromBase64(str64)  
    local b64chars = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/'  
    local temp={}  
    for i=1,64 do  
...
--url安全的base64
function ExternalFun.urlSafeBase64(strBase64)
...
--异或加密字符串(返回base64字符串)
function ExternalFun.xorEncrypt(str, key)
    ...
    local strBase64 = ExternalFun.base64FromBytes(dataBytes)
