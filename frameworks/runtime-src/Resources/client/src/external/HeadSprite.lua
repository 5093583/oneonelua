...
function HeadSprite.checkData(useritem)
	...
	if useritem.wFaceID > yl.FACE_COUNT - 1 then
		useritem.wFaceID = 0
	end
...
--创建裁剪玩家头像
function HeadSprite:createClipHead( useritem, headSize,client ,clippingfile )
	if nil == useritem then
		--return
	end
	useritem = HeadSprite.checkData(useritem)
	...
	if nil ~= clipSp then
        ...
        clipSp:setScale(SYS_HEADSIZE / clipSp:getContentSize().width)

		--裁剪
		local clip = cc.ClippingNode:create()
		clip:setStencil(clipSp)
        clip:setAlphaThreshold(0.5) --2017.5.27 改为0.5更圆滑
...
--创建裁剪玩家头像
function HeadSprite:createGameClipHead( useritem, headSize ,clippingfile )
	if nil == useritem then
		--return
	end
	useritem = HeadSprite.checkData(useritem)
...
function HeadSprite:createSysHead( faceId, headSize, clippingfile )
    ...
    --检查数据
    local sysHeadCount = HeadSprite:getSysHeadCount()
    if (faceId > sysHeadCount - 1) then
        faceId = sysHeadCount - 1
    end
...
function HeadSprite:updateHead( useritem )
   ...
   -- print("更新用户头像"..useritem.dwUserID..useritem.bThirdPartyLogin..useritem.bThirdPartyLogin)
    local bMySelfInfo = (useritem.dwUserID == GlobalUserItem.dwUserID)
    if bMySelfInfo==true then
        --useritem.bThirdPartyLogin=GlobalUserItem.bThirdPartyLogin
        --useritem.szThirdPartyUrl=GlobalUserItem.szThirdPartyUrl
        
    end
    isThirdParty=useritem.bThirdPartyLogin
	...
	if isThirdParty and nil ~= useritem.dwCustomID and 0 ~= useritem.dwCustomID and false == bAntiCheat and useritem.wFaceID==0 then
		--判断是否有缓存
    ...
    if isThirdParty and nil ~= useritem.szThirdPartyUrl and string.len(useritem.szThirdPartyUrl) > 0 and useritem.wFaceID<0 then
	...
	print("更新系统头像".. self.m_headSize..SYS_HEADSIZE)
	self.m_fScale =( self.m_headSize-20) / SYS_HEADSIZE
...
function HeadSprite:enableHeadFrame( bEnable, frameparam )
	...
	--根据会员等级配置
	local vipIdx = 5 --self.m_useritem.cbMemberOrder or 0
	local framestr = string.format("sp_frame_%d_0.png", vipIdx)
	local deScale = 0.72
...
function HeadSprite:initHeadSprite( useritem )
    ...
    --print("初始化头像"..useritem.dw)
	self.m_useritem = useritem
	local isThirdParty = useritem.bThirdPartyLogin or false	
	isThirdParty = false -- 不使用第三方头像

    print("HeadSprite:initHeadSprite,"..useritem.dwCustomID.." isThirdParty:" .. tostring(isThirdParty) .. ", szThirdPartyUrl:" .. tostring(useritem.szThirdPartyUrl))
	       ...
	       if nil ~= frame then
			...
			self:downloadFace(url, path, filename, function(downloadfile)
				...
				if selffile == downloadfile then
            	    ...
            	    function g_FaceResizeListener(event)
            			print("resize")
            			--是否是自己文件
                        if true then
                            local customframe = cc.Sprite:create(filepath):getSpriteFrame()
                            --缓存帧  								
                            ...
                            self:setScale(1)
                            local selfSize = self:getContentSize()
                            print("width " .. selfSize.width .. " height " .. selfSize.height)
                            cl:onUpdateUserInfo(1)
							...
							uploader:uploadFile(function(sender, ncode, msg)
                                ...
                                local jdata=cjson.decode(msg)
                                GlobalUserItem.dwCustomID=jdata["data"]["CustomID"]
                                print("上传更新自定义头像"..GlobalUserItem.dwCustomID)
                    ...
                    reSizeGivenFile(filepath, filepath, "g_FaceResizeListener", 96)
            	end
			end)			
		end
	elseif isThirdParty and nil ~= useritem.dwCustomID and 0 ~= useritem.dwCustomID then
		--判断是否有缓存或者本地文件
		local framename = useritem.dwUserID .. "_custom_" .. useritem.dwCustomID .. ".ry"
			...
			self:downloadFace(url, path, filename, function(downloadfile)
	        	...
	        	if selffile == downloadfile then
	        		...
	        		if nil ~= customframe then
						...
						cc.SpriteFrameCache:getInstance():addSpriteFrame(customframe, framename)
						customframe:retain()
						yl.ClientScene:onUpdateUserInfo()
						--self:updateHeadByFrame(customframe)
					end
	        	end
			end)
...
function g_FaceDownloadListener(code, msg, filename)
    print("jjjjjj")
        if nil ~=filename and 0 == code then
        	if nil ~= onDownLoadSuccess 
        		and type(onDownLoadSuccess) == "function" 
        		and nil ~= filename 
        		and type(filename) == "string" then
...
--    	self.m_downListener = cc.EventListenerCustom:create("d_image",eventCustomListener)
