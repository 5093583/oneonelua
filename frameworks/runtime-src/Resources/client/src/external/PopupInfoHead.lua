...
function PopupInfoHead:createGameClipHead( useritem, headSize, clippingfile )
	local sf = PopupInfoHead.new()
	local head = HeadSprite:createGameClipHead(useritem, headSize, clippingfile)
	sf.m_useritem = useritem
	sf.m_head = head
	if nil ~= head then
		sf:addChild(head)
...
function PopupInfoHead:getInfoLayerPos(  )
    ...
    print(size.width * 0.11, size.height * 0.26)
