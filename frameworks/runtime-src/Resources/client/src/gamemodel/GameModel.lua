...
function GameModel:ctor(frameEngine,scene)
    ...
    if nil ~= yl.ClientScene then
	    yl.ClientScene._gameView=self
    end
...
function GameModel:onExitTable()
    print("退了桌子GameModel:onExitTable")
    self:stopAllActions()
    self:KillGameClock()
...
function GameModel:OnUpdataClockView()
    if self._gameView and self._gameView.OnUpdataClockView then
        self._gameView:OnUpdataClockView(self._ClockViewChair,self._ClockTime)  --改变时间
    end
end
