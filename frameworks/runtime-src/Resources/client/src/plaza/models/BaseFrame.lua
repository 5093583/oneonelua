...
function BaseFrame:onSocketCallBack(pData)
	...
	print("-----------------------------网络消息回调onSocketCallBack main:"..main.."#sub:"..sub.." len:"..pData:getlen())	
...
function BaseFrame:sendSocketData(pData)
    print("发送数据BaseFrame:sendSocketData(pData)")
    dump(pData,"退出发送的数组")
	if self._socket == nil then
		self._callBack(-1)
		return false
    ...
    print("------------发送数据sendSocketData------------>main|:"..pData:getmain().." sub:"..pData:getsub())
    print("------------发送数据sendSocketData------------>len|:"..pData:getlen().." nCurGameKind:".. GlobalUserItem.nCurGameKind)
