...
function GameFrameEngine:addShotFrame(shotFrame)
    ...
    if appdf.isObject(shotFrame) then
        ...
        table.insert(self._shotFrame, shotFrame)
    end
end
...
function GameFrameEngine:removeShotFrame(shotFrame)
    ...
    for i = 1, #self._shotFrame do
        if self._shotFrame[i] == shotFrame then
            if appdf.isObject(shotFrame) then
                shotFrame._gameFrame = nil
            end
            table.remove(self._shotFrame, i)
            return
        end 
    end
end
...
--		self._socket:setdelaytime(0)
        self._socket:setdelaytime(86400000)
...
--        PriRoom:getInstance():onEnterPlaza(nil, self)
...
function GameFrameEngine:SitDown(table ,chair,password)
    if GlobalUserItem.tabEnterGame._KindID == "2010" then
--    if GlobalUserItem.tabEnterGame._KindID == "26" or GlobalUserItem.tabEnterGame._KindID == "2010" then
--        	GlobalUserItem.clientScene._gameFrame:onCloseSocket()
            self:onCloseSocket()
            dismissPopWait()
            callClientC("setDataType,dataLoad")
--            GlobalUserItem.clientScene:onChangeShowMode()
            local passwordStr = "";
            if password then
                passwordStr = password;
            end
            GlobalUserItem.clientScene.isInCGame =  true
            
            callClientC("openTable,"..GlobalUserItem.tabEnterGame._KindID..","..self._viewFrame._roomInfo._nRoomIndex..","..table..","..chair..","..passwordStr)
...
function GameFrameEngine:onBackground()
	if self._viewFrame and self._viewFrame.onBackground  then
		self._viewFrame:onBackground()
	end
end
