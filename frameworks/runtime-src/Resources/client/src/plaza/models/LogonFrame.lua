...
function LogonFrame:onSubLogonEvent(sub,pData)
	...
	elseif sub == yl.SUB_MB_LOGON_FAILURE then
		...
		if 10 == cmdtable.lResultCode then
			...
			if nil ~= self._callBack then
				self._callBack(cmdtable.lResultCode,cmdtable.szDescribeString)
			end
        elseif 55 == cmdtable.lResultCode then
            print("手机验证")
            --需要进行手机验证
    	    if nil ~= self._callBack then
				self._callBack(cmdtable.lResultCode,cmdtable.szDescribeString)
...
function LogonFrame:onLoginByThirdParty(szAccount, szNick, cbgender,openid,headurl, platform)
	--数据保存
	self._szAccount = szAccount
	self._szNickName = szNick
	self._cbLoginGender = cbgender
	self._cbPlatform = platform
	self._szopenid = openid
	self._szheadurl = headurl
...
function LogonFrame:onRegister(szAccount,szNickName,szPassword,cbGender,szMobile,szSMSCode)
	--数据保存
	self._szRegAccount = szAccount
    self._szRegNickName = szNickName
	self._szRegPassword= szPassword
	self._cbRegGender = cbGender
	self._szMobilePhone = szMobile
    self._szSMSCode = szSMSCode
...
function LogonFrame:sendLogon()
	local LogonData = CCmd_Data:create(235+14)
    ...
    LogonData:pushstring(self._szSMSCode,yl.LEN_SMS_CODE)
...
function LogonFrame:sendThirdPartyLogin( )
	...
	cmddata:pushstring(self._szopenid,33) --openid
	cmddata:pushstring(self._szheadurl,200) --headurl
    ...
    cmddata:pushstring(self._szSMSCode,yl.LEN_SMS_CODE)
...
function LogonFrame:sendRegister()
    ...
    RegisterData:pushstring(self._szSMSCode,yl.LEN_SMS_CODE)
