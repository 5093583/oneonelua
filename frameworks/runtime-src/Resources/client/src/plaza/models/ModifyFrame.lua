...
function ModifyFrame:sendAccountRegisteBinding()
	local AccountBinding = CCmd_Data:create(331+64) --4+66+66+64+66+64+1   + 64
	...
	AccountBinding:pushstring(self._szAccount,32)
    AccountBinding:pushstring(self._szNickName,32)
	AccountBinding:pushstring(self._szPassword,33)
...
function ModifyFrame:sendModifySystemFace( )
    print("ModifyFrame:sendModifySystemFace")
    ...
    print(GlobalUserItem.szPassword.."sendModifySystemFace"..md5(GlobalUserItem.szPassword));
...
function ModifyFrame:onAccountRegisterBinding(szAccount, szNickName, szPassword, szSpreader)
	--数据保存
	self._szAccount = szAccount
    self._szNickName = szNickName
	self._szPassword = szPassword
