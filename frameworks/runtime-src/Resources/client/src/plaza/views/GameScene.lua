...
function GameScene:ctor(delegate)
    ...
    print("GameScene:ctor2")
	if nil ~= entergame then
        print("createGameLayer")
...
function GameScene:createGameLayer()
    ...
    if PriRoom and nil ~= self._gameLayer and true == GlobalUserItem.bPrivateRoom then
		 PriRoom:getInstance():enterGame(self._gameLayer, self)
	end
