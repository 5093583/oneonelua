...
function LogonScene:onCreate()
    ...
    --加载csb资源
    local rootLayer, csbNode = ExternalFun.loadRootCSB("plaza/LogonLayer.csb", self)

    --隐藏美女
    csbNode:getChildByName("showgirl"):setVisible(false)

    self.button_node = csbNode:getChildByName("Button_Node")
    self.agreement_node = csbNode:getChildByName("Agreement_Node")

    --用户协议
    self.cb_agreement = self.agreement_node:getChildByName("Agreement_CheckBox")

    	 --微信登录
	    self._btnLogonWx = self.button_node:getChildByName("Wechat_Btn")
        self._btnLogonWx:setVisible(false)
	    self._btnLogonWx:addClickEventListener(function() self:onClickWx() end)


	    --手机登录
	    self._btnLogonAccount = self.button_node:getChildByName("Phone_Btn")
        self._btnLogonAccount:setVisible(false)
	    self._btnLogonAccount:addClickEventListener(function() self:onClickAccount() end)


	    --游客登录
	    self._btnLogonVisitor = self.button_node:getChildByName("Visitor_Btn")
