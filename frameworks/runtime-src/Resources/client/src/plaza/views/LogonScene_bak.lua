...
function LogonScene:onCreate()
    ...
    --背景
    cc.Sprite:create("base/res/background.png")
...
--			end
    ...
    --用户协议 370
    local option_sp = cc.Sprite:create("Logon/logon_user_option.png")
        :setPosition(appdf.WIDTH/2 + 10, 50)
        :addTo(self)
    self._btnOptionSelected = ccui.Button:create("Logon/logon_user_option_1.png", "Logon/logon_user_option_1.png")
    	:setVisible(true)
    	:setPosition(-25, 12)
    	:addTo(option_sp)
    self._btnOptionUnselected = ccui.Button:create("Logon/logon_user_option_0.png", "Logon/logon_user_option_0.png")
    	:setVisible(false)
    	:setPosition(-25, 12)
    	:addTo(option_sp)
	self._btnOptionSelected:addClickEventListener(function ()
		self._btnOptionSelected:setVisible(false)
		self._btnOptionUnselected:setVisible(true)
	end)
	self._btnOptionUnselected:addClickEventListener(function ()
    		self._btnOptionSelected:setVisible(true)
    		self._btnOptionUnselected:setVisible(false)
	end)

    --提示文本
    local tip = "版本V " .. appdf.BASE_C_VERSION .. "." .. (self:getApp()._version:getResVersion() or appdf.BASE_C_RESVERSION)
    ...
    self._btnLogonVisitor:setPosition(appdf.WIDTH / 2+280, 260)
    ...
    self._btnLogonAccount:setVisible(false)
...
--    end
    ...
    --elseif yl.SHOW_MOBILE_BINDING_TIP == nil then
     ...
     --   showToast(self, "为了您的账号安全，未绑定手机的玩家，请在设置进行手机绑定！", 5)
    end
end
function LogonScene:onKeyBack()
...
--    end
    ...
    for i = 1, #logonBtns do
        ...
        -- logonBtns[i]:setPosition(xStart + (i - 1) * 380, 260)
    end
...
function LogonScene:onClickWx()
	if not self._btnOptionSelected:isVisible() then
		--showToast(nil, "请勾选游戏协议使用许可协议", 2)
        QueryExit:create("请勾选游戏协议使用许可协议", nil, nil, QueryExit.QUERY_SURE):addTo(self)
		return
	end
    ...
    --平台判定
	local targetPlatform = cc.Application:getInstance():getTargetPlatform()
    --targetPlatform=cc.PLATFORM_OS_ANDROID
...
--            self._logonFrame:onLoginByThirdParty(self._tThirdData.accounts, self._tThirdData.nickname, self._tThirdData.gender,self._tThirdData.openid,self._tThirdData.headurl, self._tThirdData.platform)
--            return
--        end
        ...
        local function loginCallBack ( param )
		    ...
		    if type(param) == "string" and string.len(param) > 0 then
			    ...
			    if ok and type(datatable) == "table" then
				    ...
				    self._logonFrame:onLoginByThirdParty(accounts, nickname, gender,openid,headurl, yl.PLATFORM_LIST[yl.ThirdParty.WECHAT])
			    end
		    end
	    end
        --loginCallBack([[{"valid":true,"unionid":"oPzdC0tpvGV82-1XBPRdN6DmL2ro","province":"重庆","gender":"1","openid":"oIeWp0eiP9rxvSNMeETb-dDyHdDA","screen_name":".....","language":"zh_CN","profile_image_url":"http:\/\/wx.qlogo.cn\/mmopen\/vi_32\/wFNGVtxc2g0N0NTCJYqvUbjjhFNGu745L1sR6N9bZ9AfENQyMA1icFic219y3DlKmC5yv3CliaXLmiafenzzT2gEhg\/0","um_code":2,"city":"沙坪坝","country":"中国"}]])
...
function LogonScene:onLogonCallBack(result, message)
    ...
    if result ~= 1 then
        dismissPopWait()
        print("=========== 登录错误值 ==========="..message)
        -- QueryExit:create(message, nil, nil, QueryExit.QUERY_SURE):addTo(self)
    end
    ...
    if result == 1 then --成功
		...
		if self._Operate == 2 then 					--游客登录
			...
			GlobalUserItem.szPassword = "yk.pass.game316.com"
		...
		if yl.HTTP_SUPPORT then
			...
			appdf.onHttpJsionTable(yl.HTTP_URL .. "/WS/MobileInterface.ashx","GET","action=GetMobileShareConfig&userid=" .. GlobalUserItem.dwUserID .. "&time=".. ostime .. "&signature=".. GlobalUserItem:getSignature(ostime),function(jstable,jsdata)
                ...
                QueryExit:create(str, nil, nil, QueryExit.QUERY_SURE):addTo(self)
