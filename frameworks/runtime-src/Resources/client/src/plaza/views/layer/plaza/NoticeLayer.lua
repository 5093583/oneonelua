...
--加载公告页面
function NoticeLayer:loadNoticeView()
    print("加载公告页面")
    local list_ranking = self._content:getChildByName("List_Ranking")
    local item_rank = self._content:getChildByName("Item"):setVisible(false)
    ...
    if count>0 then
        for k,v in pairs(self._list) do
            ...
            item:getChildByName("text"):setString(v.Subject)
            item:getChildByName("time"):setString("2018-04-29")
...
function NoticeLayer:requestNoticeList()
    ...
    appdf.onHttpJsionTable(url ,"GET","action=getmobilenotice",function(jstable,jsdata)
