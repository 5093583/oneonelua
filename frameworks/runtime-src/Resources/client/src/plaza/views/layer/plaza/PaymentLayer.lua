...
local KefuPayLayer = appdf.req(appdf.CLIENT_SRC .. "plaza.views.layer.plaza.KefuPayLayer")
...
function PaymentLayer:ctor(listType,amount, count, appid, callback)
    ...
    local str="金币"
    if(ListType.Gold==listType) then
    str="金币"
    elseif(ListType.RoomCard==listType)then
    str="房卡"
    end
    --设置商品名称
    self._txtName:setString(count .. str)
    --设置价格
    ...
    --人工充值
    local btnKefu = self._content:getChildByName("btn_kefu")
    btnKefu:addClickEventListener(function()
        local backsce=KefuPayLayer:create()
