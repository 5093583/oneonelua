...
function ShopLayer:ctor(listType)
    ...
    --审核隐藏
    local txtDescription = csbNode:getChildByName("txt_description")
...
--        self:removeFromParent()
        -- 把返回都改成这样 
        self:onKeyBack() --用这个封装一下 统一一个 名字 
    end)
    ...
    --2017.8.13 暂时游戏豆
    self._btnCategorys[ListType.Bean]:setVisible(false)

    --充值卡兑换
    local btnCardPay = csbNode:getChildByName("btn_card_pay")
    -- btnCardPay:setVisible(not yl.APPSTORE_VERSION)
    btnCardPay:setVisible(false)
    ...
    --列表容器
    self._listContainer = csbNode:getChildByName("list_container")
    ...
    --选中指定分类
    self:onClickCategory(listType or ListType.Gold)
...
function ShopLayer:onClickPrice(listType, index, price, count, itemid, productid)
    ...
    print("点击价格按钮", listType, index,itemid)
    ...
    if true then
        ...
        if yl.APPSTORE_VERSION and (targetPlatform == cc.PLATFORM_OS_IPHONE or targetPlatform == cc.PLATFORM_OS_IPAD) then
            ...
            --显示其他支付页面
            showPopupLayer(
                PaymentLayer:create(listType,price, count, itemid, function(result)
...
function ShopLayer:getListCount(listType)
    ...
    --房卡
    elseif listType == ListType.RoomCard then
        return GlobalUserItem.tabShopCache.shopRoomCardList and #GlobalUserItem.tabShopCache.shopRoomCardList or 0
    end
...
function ShopLayer:getListItem(listType, index)
    ...
    if index == self:getListCount(listType)+1 then
            flagVisible = true
            flagText = "全部兑换"
            flagFontSize = 22
    elseif listType == ListType.Bean then               --游戏豆
...
--        else
    ...
    elseif listType == ListType.RoomCard then       --房卡
        ...
        local itemInfo = GlobalUserItem.tabShopCache.shopRoomCardList[index] --以第一个配置为基础，后面的只是数量上的差别

        itemCount = itemInfo.Cash 
        itemPrice = itemInfo.BuyResultsGold
        itemId = itemInfo.ID
    ...
    --背景title
    local itemtitle = cc.Sprite:create("Shop/sp_item_bg_title.png")
    itemtitle:setPosition(itemSize.width / 2, 181)
    itemtitle:addTo(item)
    --商品图标
    ...
    iconItem:setPosition(itemSize.width / 2, 105)
    iconItem:addTo(item)
    ...
    txtItemCount:setPosition(itemSize.width / 2, 185)
    ...
    flagBg:setPosition(56,151)
    flagBg:setVisible(false)
    flagBg:addTo(item)
    ...
    btnPrice:setPosition(itemSize.width / 2, 14)
    btnPrice:addTo(item)
...
--    end)
    ...
    local btn = ccui.Button:create("Shop/sp_item_bg.png", "Shop/sp_item_bg.png")
    ...
    --价格按钮内容
    if index == self:getListCount(listType)+1 then
    local editBox = ccui.EditBox:create(cc.size(108, 36), "")
		:setPosition(75, 25)
        :setFontSize(36)
        :setText(998)
        ...
        :setPlaceholderFontSize(36)
        :setPlaceholderFontName("fonts/round_body.ttf")
		:addTo(btnPrice) 
    else
    local txtPrice = ccui.Text:create("￥" .. itemPrice, "fonts/round_body.ttf", 36)
    txtPrice:setPosition(75, 25)
...
function ShopLayer:updateList(listType)
    ...
    local itemSize          =   cc.size(241, 238)
