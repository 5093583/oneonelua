...
--     self:loadListView()
    ...
    --绑定按钮事件
    for i=1,4 do
        self["btn_open_"..i] = self._content:getChildByName("btn_open_"..i)
        self["btn_open_"..i]:addClickEventListener(function()
                                self:onClickListItem(i)
                            end)
        self["btn_open_"..i]:setVisible(false)
...
--                if true == valid then
--                    self:createQrcode(sjstable["msg"])
--                    return
--                end
--            end
--        end
...
--创建二维码
function WelfareLayer:createQrcode( url )
	...
	local function onJsionTable()
	    ...
	    if xhr.readyState == 4 and (xhr.status >= 200 and xhr.status < 207) then
            ...
            if response ~= nil then
                ...
                local f = io.open(savePath.."Qrcode.png", "wb")
                if f then
                    f:write(response)
                    f:close()

                    local code = cc.Sprite:create(savePath.."Qrcode.png")
                    self._content:addChild(code)
                    code:setPosition(cc.p(543.5, 300))
                end
            end
	    else
...
function WelfareLayer:updateListItem(index)
    ...
    self["btn_open_"..index]:setVisible(true)
end
...
--全局通知函数
cc.exports.d_image = function (ncode, msg, filename)
	print(msg)
	local event = cc.EventCustom:new("d_image")
	event.code = ncode
	event.msg = msg
...
function WelfareLayer:onKeyBack()
...
--下载头像
function WelfareLayer:downloadFace(url, path, filename, onDownLoadSuccess)

	local downloader = CurlAsset:createDownloader("d_image",url)			
