...
function RoomLayer:ctor(delegate)
	...
	delegate._gameFrame=self._gameFrame
...
function RoomLayer:resetRoom()
    ...
    self._gameInfo = nil
    self._roomInfo = nil
    local popup=cc.Director:getInstance():getRunningScene():getChildByName(POP_LAYERNAME_GAME)
    if popup~=nil then
...
function RoomLayer:logonRoom(wKindID, wServerID)
	...
	self._delegate:updateEnterGameInfo(gameInfo)
	self._delegate:updateEnterRoomInfo(roomInfo)
...
function RoomLayer:onGameFrameCallBack(result, message)
    print("回调RoomLayer:onGameFrameCallBack")
    local bClose = false
...
function RoomLayer:onExitRoom(code, message)
    ...
    print("离开房间RoomLayer:onExitRoom()")
...
function RoomLayer:onEventTableStatus(wTableID, cbTableStatus)
	print("桌子状态RoomLayer:onEventTableStatus")
    if self._roomViewLayer ~= nil then
...
function RoomLayer:onRequestFailure(code, message)
    ...
    if type(message) == "string" and message ~= "" then
        ...
        --QueryDialog:create(message, nil, nil, QueryDialog.QUERY_SURE):addTo(self._delegate._roomListLayer._roomLayer)
    end
end
