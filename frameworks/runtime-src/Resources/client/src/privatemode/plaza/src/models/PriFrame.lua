...
function PriFrame:onCreateRoom()
    print("创建房间PriFrame:onCreateRoom")
    --操作记录
    self._oprateCode = PriFrame.OP_CREATEROOM   --定义操作代码为创建房间
    if not self:onCreateSocket(yl.LOGONSERVER,yl.LOGONPORT) and nil ~= self._callBack then  --建立连接
        self._callBack(LOGINSERVER(-1),"建立连接失败！")
    end
    -- 动作定义
    PriRoom:getInstance().m_nLoginAction = PriRoom.L_ACTION.ACT_CREATEROOM    --这是什么？
end
...
function PriFrame:onConnectCompeleted()
    print("================== 连接结果PriFrame:onConnectCompeleted ================== ==> " .. self._oprateCode)
...
function PriFrame:sendCreateRoom()
    print("发送创建房间PriFrame:sendCreateRoom")
...
--        end
	    ...
	    dataBuffer:pushword(self._gameFrame:GetTableID())
	    dataBuffer:pushword(self._gameFrame:GetChairID())
	    dataBuffer:pushbyte(1)
        if not self._gameFrame:sendSocketData(dataBuffer) then
          self._callBack(GAMESERVER(-1),"退出游戏失败！")
       end
    end
end
...
function PriFrame:onSocketEvent( main,sub,pData )
    print("网络信息(短连接)PriFrame:onSocketEvent ==> " .. main .. "##" .. sub)
    ...
    if 0 == cmd_table.dwServerID then
        if nil ~= self._callBack then
            ...
            if nil ~= runScene then
                showToastNoFade(runScene, "该房间ID不存在, 请重新输入!", 2)
            end
...
function PriFrame:onSubGameServerResult(pData)
    print("创建结果PriFrame:onSubGameServerResult")
    ...
    print("发送登陆私人房"..cmd_table.dwServerID)
    PriRoom:getInstance():onLoginRoom(cmd_table.dwServerID)
    return false
end
