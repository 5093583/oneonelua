...
local PrivateFrame = appdf.req(MODULE.PLAZAMODULE .."models.PrivateFrame")
...
--    self._priFrame = PrivateFrame:create(self, privateCallBack)
...
function PriRoom:getInstance( )
    print("实现单例yes,PriRoom:getInstance")
...
function PriRoom:onLoginRoom( dwServerID, bLockEnter )
    print("登陆私人房PriRoom:onLoginRoom")
...
--        end
    ...
    if nil ~= self._priFrame and nil ~= self._priFrame._gameFrame then
        ...
        self._scene._roomLayer:logonRoom(GlobalUserItem.nCurGameKind, dwServerID)
...
function PriRoom:onLoginPriRoomFinish()
    print("PriRoom:onLoginPriRoomFinishyes")
    
    local bHandled = false
    if self.m_nLoginAction == L_ACTION.ACT_CREATEROOM and nil ~= self._viewFrame and nil ~= self._viewFrame.onLoginPriRoomFinish then
    ...
    if not bHandled then
        ...
        if (meUser.cbUserStatus == yl.US_FREE or meUser.cbUserStatus == yl.US_NULL) then  --如果用户属于站立或没有状态
            -- 搜索登陆
...
--                self._scene:onKeyBack()
        ...
        elseif meUser.cbUserStatus == yl.US_PLAYING or meUser.cbUserStatus == yl.US_READY or meUser.cbUserStatus == yl.US_SIT then
            ...
            if PriRoom:getInstance().m_nLoginAction == PriRoom.L_ACTION.ACT_SEARCHROOM
                ...
                self:showPopWait()
                -- 切换游戏场景
                print("切换游戏场景",self._scene)
                self._scene._roomLayer:onEnterTable()
                -- 发送配置
...
function PriRoom:popMessage(message, notShow)
    ...
    if type(message) == "string" and "" ~= message then
        ...
        elseif nil ~= self._viewFrame then
            if self._viewFrame.m_rootLayer ~= nil then
                showToast(self._viewFrame:getParent():getParent():getParent(), message, 2)            
            else
                print("显示priroom 消息:"..message)
                --QueryDialog:create(message, nil, nil, QueryDialog.QUERY_SURE):addTo(self._viewFrame)
...
function PriRoom:onPrivateGameServerMessage(result, message, dataBuffer, notShow)
    print("消息PriRoom:onPrivateGameServerMessage")
...
--            curTag = self._scene._sceneRecord[#self._scene._sceneRecord]
--        end
        if self._scene._roomLayer._gameScene ~= nil then
...
function PriRoom:getTagLayer(tag, param, scene)
    ...
    elseif LAYTAG.LAYER_CREATERESULT == tag then
        ...
        if nil ~= self._scene then
            ...
            print("弹窗测试弹窗测试弹窗测试弹窗测试",self._scene._roomListLayer.aaaa)
----                      dump(self_scene..aaaa)
--            print(self._scene._roomListLayer.__cname)
--            dump(self._scene._roomListLayer,"演示这里roomListLayer")
            self._scene:addChild(roomResLayer)
--showPopupLayer2(self._scene,roomResLayer)
--            showPopupLayer(self._scene._roomListLayer)
--            self._scene._roomListLayer.m_roomCreateLayer:addChild(roomResLayer)
--                        self._scene._roomListLayer:addChild(roomResLayer)
            --self._scene:addChild(roomResLayer)
    ...
    elseif LAYTAG.LAYER_FRIENDLIST == tag then
        ...
        if nil ~= runScene then
            ...
            runScene:addChild(friendshare,yl.MAX_INT - 1)
        end
    end
end
...
function PriRoom:enterRoom( scene )
    ...
    if nil ~= entergame and true == bPirMode   then--""and == self._gameSearchPath
        local modulestr = string.gsub(entergame._KindName, "%.", "/")
...
--            self._scene._roomLayer:showPopWait()
            showPopWait()
    end
end
...
--        self._scene._roomLayer:dismissPopWait()
        dismissPopWait()
    end
end
    ...
    if 1 == PriRoom:getInstance().cbIsJoinGame 
        ...
        QueryDialog:create(tip, function(ok)
                if ok == true then
                    ...
                    --if nil~=self._scene._roomLayer._roomcreatelayer then
                    --self._scene._roomLayer._roomcreatelayer:onKeyBack()
                    
                    --self._scene._roomLayer:resetRoom()
                   -- end
                    self:exitRoom()
...
--        self._scene:onKeyBack()
                ...
                --end
                self:exitRoom()
            --self._scene._roomLayer:onKeyBack()
--            
        return
    end
...
function PriRoom:getInviteShareMsg( roomDetailInfo )
    local entergame = GlobalUserItem.tabEnterGame--self._scene:getEnterGameInfo()
    if nil ~= entergame then
        local modulestr = string.gsub(entergame._KindName, "%.", "/")
        local gameFile = ""
        if cc.PLATFORM_OS_WINDOWS == targetPlatform then
