...
local RoomDetailLayer = appdf.req(appdf.PRIVATE_SRC .. "plaza.src.views.RoomDetailLayer")
...
function RoomRecordLayer:tableCellAtIndex(view, idx)	
    ...
    if nil == cell then
        ...
        for i = 1, columns[tag] do
            ...
            local txtColumn = ccui.Text:create(contents[tag][i], "fonts/round_body.ttf", 22)
            txtColumn:setTag(i)
            txtColumn:setPosition(posXs[tag][i], 30)
            txtColumn:setContentSize(widths[tag][i], 20)
            txtColumn:setColor(cc.c3b(90, 90, 85))
    ...
    elseif tag == 2 then
        local scorestr = "+" .. item.lScore
        local dismissTime = ""
        if item.lScore < 0 then
            scorestr = "" .. item.lScore
        end
        if item.bFlagOnGame then
            scorestr = ""
            dismissTime = "游戏中"
        else 
            dismissTime = os.date("%Y/%m/%d %H:%M:%S", item.sortTimeStmp)
        end
        contents = { os.date("%Y/%m/%d %H:%M:%S", item.sortCreateTimeStmp), item.szRoomID, item.szUserNicname, scorestr, dismissTime}
    end
...
function RoomRecordLayer:tableCellTouched(view, cell)
    --播放音效
    ExternalFun.playClickEffect()
    local viewTag = view:getTag()
    local tabDetail = self._recordLists[viewTag][cell:getTag()]
    if viewTag == 1 then
        if tabDetail.cbIsDisssumRoom == 1 then
            tabDetail.onGame = false
        else 
            tabDetail.onGame = true
        end
    else 
        tabDetail.onGame = tabDetail.bFlagOnGame
    end
...
--        self._recordLists[1] = PriRoom:getInstance().m_tabCreateRecord
--    else 
--        self._recordLists[2] = PriRoom:getInstance().m_tabJoinRecord
--    end
--    self._recordTableViews[self.m_selectIndex]:reloadData()
    self:onSelectTab(self.m_selectIndex, false)
...
function RoomRecordLayer:onKeyBack()
...
function RoomRecordLayer:onGetJoinRoomList(joinRoomList)
    ...
    self._recordLists[2] = joinRoomList
